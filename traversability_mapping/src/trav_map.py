...
class PointCloudMerger:
    def __init__(self):
        ...
        self.liosam_distance_threshold = 0.5
    ...
    def liosam_map_callback(self, map_msg):
        ...
        rospy.loginfo("Recieved and processed lio-sam map")

    def pointcloud_callback(self, cloud1_msg, cloud2_msg):
        if self.liosam_map is None:
            rospy.logwarn("LIO-SAM map not recieved yet. Skipping this pointcloud")
            return
        ...
        try:
            ...
            # Filter pointclouds based on LIO-SAM map
            cloud1 = self.filter_pointcloud(cloud1)
            cloud2 = self.filter_pointcloud(cloud2)
    ...
    def filter_pointcloud(self, pointcloud):
        distances, _ = self.liosam_tree.query(pointcloud[:, :3])
        return pointcloud[distances < self.liosam_distance_threshold]
