...
# this script takes a depth image, generates the pointcloud from it, and then filters the pointclouds based on masks. Outputs points with one consistent color
import numpy as np
...
def visualize_point_cloud(points, color=[1.0, 0.0, 1.0]):
    point_cloud = o3d.geometry.PointCloud()
    point_cloud.points = o3d.utility.Vector3dVector(points)

    # create a color array with the specified color
...
visualize_point_cloud(pointcloud, color=[1.0, 0.0, 1.0])
visualize_point_cloud(filtered_points, color=[1.0, 0.0, 1.0])
